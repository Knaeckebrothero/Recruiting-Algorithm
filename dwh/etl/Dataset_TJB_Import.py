...
# with open('./Datasets/techjobs_full.csv', 'r') as file:
#     reader = csv.DictReader(file)
#     for row in reader:
#         # Insert into Dim_Employee
#         cursor.execute('''
#             INSERT INTO FACT_TJB_Employee (Age, EdLevel, Employment, Gender, MainBranch, YearsCode, Country, PreviousSalary, ComputerSkills, Employed)
#             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
#         ''', (row['Age'], row['EdLevel'], row['Employment'], row['Gender'], row['MainBranch'], row['YearsCode'],
#               row['Country'], row['PreviousSalary'], row['ComputerSkills'], row['Employed']))
#
#         employee_id = cursor.lastrowid
#         # # Handle skills in HaveWorkedWith
#         # skills = row['HaveWorkedWith'].split(", ")
#         row['HaveWorkedWith'] = row['HaveWorkedWith'].fillna('')
#
#         row["HaveWorkedWith"] = row['HaveWorkedWith'].str.split(';')
#
#         all_skills = set(skill for sublist in row['HaveWorkedWith'] for skill in sublist if skill)
#         #
...
#                 skill_id = skill_id[0]
#         #
#         #     # Insert into Fact_EmployeeSkills
#         #     cursor.execute('INSERT INTO FACT_TJB_EmployeeSkills (EmployeeID, SkillID) VALUES (%s, %s)',
